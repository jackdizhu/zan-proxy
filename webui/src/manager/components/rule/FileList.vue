<template>
  <div>
    ...
    <el-table border :data="$dc.ruleFileList">
      ...
      <el-table-column label="操作" :width="240" :context="_self">
        <template scope="scope">
          ...
          <span>
            <el-tooltip class="item" effect="dark" placement="top-start" content="pull规则">
              <el-button
                type="danger"
                size="mini"
                icon='warning'
                v-if="$_.get(scope,'row.meta.remote')"
                @click="pullRemoteFile(scope.row.name,scope.row.meta.url)">
              </el-button>
            </el-tooltip>
          </span>
          <span>
            <el-tooltip class="item" effect="dark" placement="top-start" content="push规则">
              <el-button
                type="danger"
                size="mini"
                icon='upload'
                v-if="$_.get(scope,'row.meta.remote')"
                @click="pushRemoteFile(scope.row.name,scope.row.meta.url)">
              </el-button>
            </el-tooltip>
          </span>
        </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async pullRemoteFile (name, remoteUrl) {
        const isConfirmed = await this.$confirm(
          'pull该远程规则会覆盖本地对这个规则的修改,请谨慎操作',
          '提示',
          {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
        ...
        try {
          const {data:remoteContent} = await this.$http.get(remoteUrl, { cors: true });
          await ruleApi.saveFile(name, remoteContent);
          this.$message({
            message: 'pull远程规则成功!',
            type: 'success'
          });
          ...
          this.$message({
            message: 'pull远程规则失败,请检查网络状况后重试!',
            type: 'error'
          });
        }
      },
      async pushRemoteFile (name, remoteUrl) {
        const {data:{data:content}} = await ruleApi.getFileContent(name);
        try {
          await this.$http.post(remoteUrl, content);
          this.$message({
            message: 'push远程规则成功!',
            type: 'success'
          });
